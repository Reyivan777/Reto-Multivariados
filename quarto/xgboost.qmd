...
if (!requireNamespace("xgboost", quietly = TRUE)) {
  stop(
    "El paquete xgboost no está disponible en el entorno. ",
    "Ejecuta renv::restore() para restaurar las dependencias del proyecto."
  )
}
...
El error anterior aparecía porque algunas versiones recientes de `xgboost` no exponen `best_iteration` y `best_score` como elementos directos del objeto con la misma estructura esperada. Para evitar depender de esa representación interna, el siguiente bloque obtiene la mejor iteración directamente del registro de evaluación que XGBoost genera durante el entrenamiento.

```{r}
#| label: resumen-entrenamiento-xgboost-h1
registro_entrenamiento_h1 <- as_tibble(
  modelo_xgb_h1$evaluation_log
)
...
mejor_rmse_validacion_h1 <- registro_entrenamiento_h1[
  [columna_rmse_validacion]
][indice_mejor_h1]
