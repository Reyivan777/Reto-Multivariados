...
Para esta etapa se seleccionaron las estaciones `NTE`, `NTE2`, `NO` y `NE`, concentradas en el sector norte del Área Metropolitana de Monterrey y con especial interés en San Nicolás de los Garza y sus zonas colindantes. La estación `NE` se ubica en San Nicolás de los Garza y `NTE2` corresponde a la zona UANL-San Nicolás, mientras que `NTE` y `NO` aportan mediciones cercanas desde General Escobedo y Monterrey. Esta selección permite conservar un contexto espacial relevante sin utilizar simultáneamente toda la red de SIMA, lo que reduciría la manejabilidad del conjunto de datos y aumentaría la complejidad del modelado posterior.
...
# Tabla de dimensiones por año y total
dimensiones_reporte <- dimensiones |>
  mutate(Año = as.character(Año)) |>
  bind_rows(
    tibble(
      Año = "Total",
      Registros = sum(dimensiones$Registros),
      Columnas = ncol(datos)
    )
  )
