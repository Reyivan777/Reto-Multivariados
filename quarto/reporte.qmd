...
**¿Con qué precisión puede predecirse la concentración horaria diurna de O3, entre las 10:00 y 20:00, con un horizonte de hasta 24 horas en las estaciones NTE, NTE2, NO y NE, utilizando los registros históricos de contaminantes, variables meteorológicas y variables temporales proporcionados por SIMA entre 2020 y 2025?**
...
Desarrollar y evaluar modelos capaces de **predecir la concentración horaria diurna de ozono (O3), entre las 10:00 y 20:00, con un horizonte de hasta 24 horas** en las estaciones `NTE`, `NTE2`, `NO` y `NE` de la Zona Metropolitana de Monterrey, utilizando información histórica de contaminantes, variables meteorológicas y características temporales registradas por SIMA durante el periodo 2020-2025.
...
### Estabilidad del patrón horario entre años
...
### Concentración de valores altos de O3 por hora
...
### Predictores meteorológicos y O3 futuro
...
$$Corr\left(X(t), O_3(t+h)\right), \qquad h=1,\ldots,24$$
...
### Otros contaminantes y O3 futuro
...
En términos de selección de variables, `NOX`, `NO` y `NO2` son los únicos contaminantes que muestran una señal suficientemente consistente para justificar una evaluación posterior. Sin embargo, sus porcentajes elevados de valores faltantes y la redundancia entre ellos hacen poco conveniente incorporarlos automáticamente a los tres. Como punto de partida, se evaluará al menos uno de estos indicadores, probablemente `NOX`, y se comprobará mediante validación si mejora el modelo lo suficiente para compensar la pérdida de observaciones. `CO`, `PM10` y `PM2.5` quedan como candidatos de baja prioridad por su escasa asociación lineal con el O3 futuro.
...
## Conjunto preliminar de predictores
...
Las 24 horas del día permanecen disponibles como información de origen. Sin embargo, después de definir la ventana diurna del objetivo, `construir_base_horizonte()` conserva únicamente las filas cuya **hora objetivo** se encuentre entre 10:00 y 20:00. Así, una medición nocturna puede seguir sirviendo como predictor de una hora diurna posterior, pero no se utiliza como respuesta principal del modelo.
...
### Diagnóstico final de faltantes por predictor
...
### **Reporte de modelo GAM**

Se eligió un Modelo Aditivo Generalizado como el modelo estadístico no lineal del proyecto porque conserva interpretabilidad directa de una regresión lineal múltiple, es decir, se puede visualizar y explicar el modelo, predictor por predictor. Al mismo tiempo, permite capturar relaciones curvas entre O3 y sus predictores, algo que un modelo solo lineal no puede representar. GAM sirve como complemento para XGBoost y la red neuronal, ya que puede explicar de manera más clara su funcionamiento.

**Ajuste general**

El modelo para h = 1 explica el 86.2 % de la devianza de O3 (R² ajustado = 0.861), usando siete smooths meteorológicos/temporales más efectos paramétricos de estación y dirección del viento, sobre un conjunto de 39,043 casos completos (86.6 % de los objetivos disponibles). \| \| \### Concurvity La mayoría de los smooths presenta concurvity moderada (0.42-0.66), esperable por la covariación natural entre variables meteorológicas. El rezago de 24 horas de O3 destaca con 0.963, ya que comparte con la hora del día la misma periodicidad diurna; se conserva por ser significativo (p \< 0.001), aunque su forma exacta debe interpretarse con cautela.

**Efectos no lineales**

Las curvas son consistentes con la química esperada del ozono: `O3_t` crece casi linealmente y se satura cerca de 130 µg/m³ (el predictor dominante); `O3_lag_24` refleja la persistencia diaria; `SR` (radiación solar) muestra forma de joroba, coherente con su rol como precursor fotoquímico que se satura a niveles extremos; `TEMP` crece (acelera la fotoquímica); `RH` decrece (asociada a nubosidad y remoción de contaminantes); `WS` decrece y se estabiliza (dilución atmosférica). La hora objetivo pica entre 11:00-12:00 —más temprano que el pico crudo de O3 (14:00-16:00)—, lo que sugiere que los rezagos de O3 ya explican buena parte del incremento vespertino, dejando a la hora principalmente el efecto de la acumulación matutina. El mes objetivo mantiene el patrón estacional ya descrito (máximo en primavera).

**Dirección del viento y estación**

WD_sin y WD_cos son significativos (p \< 0.001) y consistentes con el patrón descriptivo (mayor O3 con viento del sureste-este) —interesante porque en XGBoost esta variable tuvo Gain casi nulo, mostrando que ambos modelos priorizan señales débiles de forma distinta. El efecto de estación (NTE como referencia) es negativo y significativo para las tres restantes, pero de magnitud pequeña frente a las diferencias brutas del análisis exploratorio, indicando que la mayoría de la variación entre estaciones se explica por sus condiciones meteorológicas locales, no por la ubicación en sí.

**Gaussiano contra Gamma**

Ante la heterocedasticidad detectada en el modelo gaussiano, se probó una familia Gamma (link log), adecuada para variables positivas con varianza creciente. El desempeño fue prácticamente idéntico (MAE 4.884 vs. 4.933) y Gamma sí corrigió la heterocedasticidad, pero se optó por reportar el gaussiano como modelo principal por interpretarse directamente en unidades de O3; Gamma se documenta como verificación de robustez.

**Desempeño por horizonte**

La mejora del GAM sobre persistencia varía marcadamente: 30.5 % (h=1), 53.9 % (h=6), 51.9 % (h=12), 11.6 % (h=24). Este patrón coincide con la correlación de O3 con sus propios rezagos (Figura 12): cae de 0.916 en h=1 hasta -0.208 cerca de 12h y se recupera a 0.741 en h=24. Donde la persistencia ya es buena predictora (h=1, h=24), el margen de mejora es limitado; donde deja de serlo (h=6, h=12), la meteorología y la hora aportan mucho más. El mismo patrón aparece, de forma independiente, en la importancia de variables de XGBoost (O3_t domina en h=1/h=24; la hora y TEMP dominan en h=6/h=12) .

Los dos modelos de naturaleza distinta coincidiendo en la misma estructura temporal, lo que hace más sólida esta conclusión sobre la dinámica del ozono en la ZMM.
