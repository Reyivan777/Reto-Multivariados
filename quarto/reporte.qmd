...
```{r}
#| label: etapa2-diccionario-variables
estaciones_seleccionadas <- c("NTE", "NTE2", "NO", "NE")

datos_etapa2 <- datos_limpios |>
  filter(Estacion %in% estaciones_seleccionadas)

diccionario_etapa2 <- tribble(
  ~Grupo, ~Variable, ~Descripcion, ~Tipo, ~Valores_posibles,
  "Contaminante", "CO", "Monóxido de carbono", "Numérica continua", "0-20 ppm*",
  "Contaminante", "NO", "Monóxido de nitrógeno", "Numérica continua", "0-500 ppb*",
  "Contaminante", "NO2", "Dióxido de nitrógeno", "Numérica continua", "0-200 ppb*",
  "Contaminante", "NOX", "Óxidos de nitrógeno", "Numérica continua", "0-500 ppb*",
  "Contaminante", "O3", "Ozono", "Numérica continua", "0-185 ppb*",
  "Contaminante", "PM10", "Partículas menores a 10 micrómetros", "Numérica continua", "0-999 µg/m3*",
  "Contaminante", "PM2.5", "Partículas menores a 2.5 micrómetros", "Numérica continua", "0-999 µg/m3*",
  "Contaminante", "SO2", "Dióxido de azufre", "Numérica continua", "0-405 ppb*",
  "Meteorológica", "BP", "Presión atmosférica", "Numérica continua", "687.5-750 mm Hg*",
  "Meteorológica", "RAINF", "Precipitación", "Numérica continua", "0-80 mm/Hr*",
  "Meteorológica", "RH", "Humedad relativa", "Numérica continua", "0-100 %*",
  "Meteorológica", "SR", "Radiación solar", "Numérica continua", "0-1.26 kW/m2*",
  "Meteorológica", "TEMP", "Temperatura", "Numérica continua", "-6.5-45.5 °C*",
  "Meteorológica", "WS", "Velocidad del viento", "Numérica continua", "0-75 Km/hr*",
  "Meteorológica", "WD", "Dirección del viento", "Numérica circular", "0-360°*",
  "Temporal/espacial", "Estacion", "Estación de monitoreo", "Categórica nominal", "NTE, NTE2, NO, NE",
  "Temporal/espacial", "Año", "Año de la medición", "Temporal discreta", "2020-2025",
  "Temporal/espacial", "fecha y hora", "Momento de la medición", "Fecha-hora", "Registros horarios 2020-2025"
)

faltantes_diccionario <- datos_etapa2 |>
  summarise(
    across(
      all_of(diccionario_etapa2$Variable),
  ...
  pivot_longer(
    everything(),
    names_to = "Variable",
    values_to = "Nulos_porcentaje"
  )

tabla_variables_etapa2 <- diccionario_etapa2 |>
  left_join(faltantes_diccionario, by = "Variable")

knitr::kable(
  tabla_variables_etapa2,
  col.names = c(
    "Grupo", "Variable", "Descripción", "Tipo",
    "Valores posibles / unidad", "% nulos"
  ),
  caption = "Descripción de las variables utilizadas en la Etapa 2"
)
```
...
#### Verificación de categorías y escritura
...
```{r}
#| label: etapa2-verificacion-categorias
estaciones_observadas <- datos_etapa2 |>
  distinct(Estacion) |>
  arrange(Estacion) |>
  pull(Estacion) |>
  as.character()

estaciones_inesperadas <- setdiff(
  estaciones_observadas,
  estaciones_seleccionadas
)

variables_esperadas <- c(
  "fecha y hora", variables_medicion,
  "Año", "Estacion"
)
...
resumen_categorias <- tibble(
  Verificacion = c(
    "Categorías observadas en Estacion",
    "Categorías inesperadas",
    "Variables esperadas faltantes"
  ),
  Resultado = c(
    paste(estaciones_observadas, collapse = ", "),
    ifelse(
      length(estaciones_inesperadas) == 0,
      "Ninguna",
      paste(estaciones_inesperadas, collapse = ", ")
    ),
    ifelse(
...
knitr::kable(
  resumen_categorias,
  caption = "Verificación de categorías y nombres de variables"
)
```
