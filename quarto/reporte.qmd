...
Los histogramas permiten comprobar visualmente si las diferencias entre media y mediana identificadas en la sección anterior corresponden a distribuciones asimétricas y si existen colas formadas por concentraciones elevadas.
...
En este grupo interesa observar especialmente la concentración de `RAINF` alrededor de cero, el comportamiento de `SR` a lo largo de periodos con y sin radiación, y la forma de las distribuciones de `TEMP`, `RH`, `BP` y `WS`. La dirección del viento (`WD`) se mantiene como un caso particular por su naturaleza circular.
...
Los valores identificados como atípicos mediante los boxplots no deben interpretarse automáticamente como errores. La limpieza previa eliminó las observaciones fuera de los rangos establecidos para cada variable y año, por lo que los valores extremos restantes pueden representar mediciones reales poco frecuentes y se conservan para los análisis posteriores.

Si los valores extremos dificultan la lectura de alguno de los diagramas de caja, se podrá utilizar posteriormente una visualización ampliada hasta el percentil 99. Este ajuste sería únicamente visual y no implicaría eliminar observaciones del conjunto de datos.
