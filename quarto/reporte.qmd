...
Esta visualización permite identificar si las concentraciones típicas y la dispersión de cada contaminante cambian entre `NTE`, `NTE2`, `NO` y `NE`. Diferencias claras entre las cajas indicarían que el comportamiento de la contaminación no es homogéneo entre los puntos de monitoreo.
...
La comparación meteorológica permite revisar si las estaciones están expuestas a condiciones distintas de presión, precipitación, humedad, radiación solar, temperatura y velocidad del viento. En el caso de `WD`, el boxplot se utiliza únicamente como descripción exploratoria, ya que la dirección del viento es una variable circular y requiere un tratamiento especial para análisis más formales.

En conjunto, esta sección permitirá determinar qué tan importante es conservar la estación de monitoreo como componente espacial del análisis. Si se observan diferencias consistentes entre estaciones, será necesario considerar esta heterogeneidad al estudiar las relaciones entre meteorología y contaminantes y durante la construcción posterior de los modelos predictivos.
...
La comparación por estación permite revisar si las asociaciones generales se mantienen en los cuatro puntos de monitoreo o si cambian según la ubicación. Una relación con signo y magnitud similares entre estaciones sugeriría un patrón relativamente estable, mientras que diferencias importantes indicarían que la estación debe considerarse explícitamente durante el modelado.

En conjunto, el análisis de correlación muestra que la meteorología no se relaciona de la misma manera con todos los contaminantes. `O3` presenta las asociaciones más claras con las variables meteorológicas analizadas, mientras que `NO`, `NO2` y `NOX` destacan principalmente por sus relaciones negativas con la velocidad del viento y la temperatura. Estos resultados proporcionan una primera guía para la selección de variables predictoras, pero deberán complementarse posteriormente con los modelos multivariados y con la evaluación de su desempeño predictivo.
...
Los valores faltantes no se eliminan de forma generalizada en esta etapa. Su tratamiento dependerá del contaminante que se seleccione como variable objetivo y de los requerimientos del modelo utilizado. Por ello, la disponibilidad de información será uno de los criterios para definir cuáles contaminantes y variables predictoras pueden utilizarse de manera confiable.
...
Esta comparación permite identificar si la falta de información se distribuye de forma similar entre estaciones o si ciertas combinaciones de variable y punto de monitoreo presentan una disponibilidad considerablemente menor. Estas diferencias deberán considerarse durante la selección del conjunto de entrenamiento.
...
Después del proceso de limpieza no permanecen mediciones fuera de los rangos definidos para cada variable y año, y la revisión de la combinación año-estación-fecha permite confirmar la ausencia de registros duplicados en el subconjunto seleccionado. Los valores extremos que permanecen dentro de los rangos válidos se conservan, ya que pueden representar observaciones reales y no errores de captura.
...
La frecuencia de registros permite evaluar si las cuatro estaciones aportan cantidades de información comparables o si existe alguna estación con una representación considerablemente menor. Esta diferencia, en caso de ser importante, deberá considerarse al comparar estaciones y durante la partición de los datos para los modelos.

En general, el conjunto seleccionado se encuentra depurado respecto a valores inválidos y duplicados. El principal aspecto de calidad que permanece es la presencia de datos faltantes, cuya magnitud puede variar entre variables y estaciones. Por esta razón, la disponibilidad de observaciones deberá considerarse junto con las relaciones encontradas en el análisis de correlación antes de seleccionar definitivamente los contaminantes y las variables predictoras. La estrategia para tratar los valores faltantes se definirá posteriormente de acuerdo con las necesidades del modelo seleccionado, evitando eliminar información de manera innecesaria.
