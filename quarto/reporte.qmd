...
resumen_faltantes <- map_dfr(
  ...
  function(variable) {
    ...
    faltantes_originales <- sum(is.na(original) | texto == "")
    no_numericos <- sum(
      !is.na(original) &
        texto != "" &
        is.na(numerico)
...
`Porcentaje_faltante` considera tanto los `NA` actuales como las observaciones con `-9999`, ya que estas últimas no representan una medición real. La tabla permite identificar qué variables tienen suficiente cobertura antes de decidir cómo tratar las ausencias.
...
En este punto todavía no se modifica ningún valor. El siguiente paso será decidir el tratamiento de `-9999` y de cualquier otra observación inválida a partir de los resultados de estas verificaciones.
