...
La decisión final sobre la ventana horaria se tomará después de revisar estos resultados. Independientemente del intervalo seleccionado, las mediciones de las 24 horas se conservarán en la base como información histórica; la posible restricción afectaría únicamente a las horas utilizadas como **objetivo y evaluación** del pronóstico.
...
La separación entre entrenamiento, validación y prueba se asigna con el **año de la hora objetivo**. De esta manera, por ejemplo, una predicción emitida al final de 2023 cuyo objetivo cae en 2024 pertenece a validación y no se filtra accidentalmente dentro del entrenamiento.

Antes de decidir cómo tratar los faltantes se cuantifica cuánta información quedaría disponible si se exigieran casos completos. Se comparan dos versiones: el conjunto principal sin otros contaminantes y la misma estructura agregando `NOX`.
...
resumen_disponibilidad_modelado |>
  ...
  knitr::kable(
    caption = "Disponibilidad de casos completos en la base formal de modelado"
  )
```

En este punto todavía **no se imputan predictores ni se eliminan filas de forma definitiva**. La tabla permitirá medir primero cuánto se pierde por exigir información completa y, en particular, cuánto reduce la muestra la incorporación de `NOX`. Con esos resultados se decidirá si el conjunto principal puede trabajarse mediante casos completos o si conviene aplicar un tratamiento específico de faltantes antes del modelado.
