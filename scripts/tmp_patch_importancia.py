...
if old_return not in texto:
    raise RuntimeError("No se encontró el retorno de entrenar_y_evaluar_horizonte")
texto = texto.replace(old_return, new_return, 1)

old_resultados = '''resultados_horizontes <- bind_rows(
  resultado_h1,
  map_dfr(c(6, 12, 24), entrenar_y_evaluar_horizonte)
) |>
  arrange(Horizonte_h)'''

new_resultados = '''evaluaciones_horizontes <- map(
  c(6, 12, 24),
...
if old_resultados not in texto:
    raise RuntimeError("No se encontró la construcción de resultados_horizontes")
texto = texto.replace(old_resultados, new_resultados, 1)
...
marcador = "\n# Evaluación\n"
if "#| label: importancia-por-horizonte" not in texto:
    if marcador not in texto:
        raise RuntimeError("No se encontró la sección Evaluación")
    texto = texto.replace(marcador, "\n" + bloque.strip() + "\n\n# Evaluación\n", 1)

texto = texto.replace(
    "La configuración inicial ya se evalúa sobre **2024** en horizontes de 1, 6, 12 y 24 horas. Estos resultados permitirán decidir el siguiente paso de ajuste sin utilizar todavía el conjunto de prueba final.",
...
Path("scripts/tmp_patch_importancia.py").unlink()
Path(".github/workflows/tmp_patch_importancia.yml").unlink()
